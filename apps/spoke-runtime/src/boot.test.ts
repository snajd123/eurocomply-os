...
describe('Spoke Boot', () => {
  ...
  it('should install packs from directory on boot', async () => {
    ...
    // Verify ComplianceLock was generated by createInstallPlan
    const locks = await spokeWithPacks.packService.listLocks({
      tenant_id: 'test-tenant-packs',
      principal: { type: 'system', id: 'test' },
      correlation_id: 'test',
    });
    expect(locks.data.total).toBe(1);
    expect(locks.data.items[0].root_pack.name).toBe('@test/boot-pack');
    expect(locks.data.items[0].packs['@test/boot-pack@1.0.0']).toBeDefined();
